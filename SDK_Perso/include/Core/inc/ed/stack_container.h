...
// 	template<typename T> 
// 	using AllocatorT = typename ed::allocator<T>;
// #else
// 	template<typename T> 
// 	using AllocatorT = typename std::allocator<T>;
// #endif
//Судя по всему VS2012 не поддерживает using, до лучших времен используется хак
#ifdef USE_NEDMALLOC_STL_ALLOCATOR
	template <typename T>
	struct AllocatorType {
		typedef typename ed::allocator<T> type;
	};
#else
	template <typename T>
	struct AllocatorType {
...
class StackAllocator : public AllocatorType<T>::type {
 public:
  typedef typename  AllocatorType<T>::type AllocatorT;
  typedef typename  AllocatorT::pointer pointer;
  ...
  struct Source {
	...
	typedef typename std::aligned_storage<sizeof(T), std::alignment_of<T>::value>::type AlignedSorage;
	AlignedSorage stack_buffer_[stack_capacity];
  ...
  pointer allocate(size_type n, const void* hint = 0) {
    if (source_ != NULL && !source_->used_stack_buffer_
      ...
      return AllocatorT::allocate(n, hint);
    }
  }
