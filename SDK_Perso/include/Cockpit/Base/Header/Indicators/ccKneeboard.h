...
class COCKPITBASE_API ccKneeboard : public ccIndicator
{
	...
	void update_camera();
	unsigned default_page;
public:
	ccKneeboard();
	virtual ~ccKneeboard();

	void initialize(avDevice* boss, unsigned char ID, const ed::string& script_name);
	...
	bool check_chunk(const ed::string & name);
	unsigned apply_image_as_page(const char *texName);//returned pageid
