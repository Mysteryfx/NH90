#pragma once

#include "ed/map.h"
#include "ed/string.h"

namespace gunner_AI {

	template<typename T>
	class AIBaseCreator
	{
	public:
		virtual T* create() const = 0;
	};

	template<class T>
	class AIFactory
	{
	public:

		static T* create(const ed::string& name) 
		{ 
			auto it = fabric.map_.find(name);
			if(it != fabric.map_.end())
				return it->second->create();
			return nullptr;
		};

		/*private:*/
		typedef ed::map<ed::string,AIBaseCreator<T>*>  AIFactorymap;
		static struct  Fmap
		{
			~Fmap()
			{
				auto it = map_.begin();
				while(it != map_.end())
				{
					delete it->second;
					++it;
				}
				map_.clear();
			}
			AIFactorymap map_;
		} fabric;
	};

	template <typename B, typename T, typename F>
	class AICreator : public AIBaseCreator<B>
	{
	protected:
		AICreator() {}
	public:
		AICreator(const ed::string& name)
		{
			auto it = F::fabric.map_.find(name);
			if(it ==  F::fabric.map_.end())
			{
				F::fabric.map_[name] = new AICreator<B,T,F>();
			}
		}

		virtual B* create() const { return new T(); }
	};

}